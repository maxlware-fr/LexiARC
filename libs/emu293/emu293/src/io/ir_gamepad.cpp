  ...
  void IRGamepadEvent(SDL_Event *ev) {
    switch (ev->type) {
      ...
      case SDL_MOUSEBUTTONUP: {
        // Souris -> manette IR virtuelle (joueur 1 uniquement).
        // Clic gauche = Sélectionner (A). Clic droit = Annuler/retour (B).
        if (ev->button.button == SDL_BUTTON_LEFT) {
          if (ev->type == SDL_MOUSEBUTTONDOWN)
            player_state[0] |= uint16_t(BTN_A);
          else
            player_state[0] &= ~uint16_t(BTN_A);
          IRGamepadUpdate(1, player_state[0]);
        } else if (ev->button.button == SDL_BUTTON_RIGHT) {
          if (ev->type == SDL_MOUSEBUTTONDOWN)
            player_state[0] |= uint16_t(BTN_B);
          else
            player_state[0] &= ~uint16_t(BTN_B);
          IRGamepadUpdate(1, player_state[0]);
        }
      ...
      case SDL_MOUSEWHEEL: {
        // Molette -> impulsion D-pad (joueur 1 uniquement).
        // Scroll haut = "aller avant" (UP). Scroll bas = "aller après" (DOWN).
        int y = ev->wheel.y;
        if (ev->wheel.direction == SDL_MOUSEWHEEL_FLIPPED)
          y = -y;
        if (y != 0) {
          int btn = (y > 0) ? BTN_UP : BTN_DOWN;
          // impulsion : presse puis relâche immédiatement (deux paquets IR consécutifs)
          player_state[0] |= uint16_t(btn);
          IRGamepadUpdate(1, player_state[0]);
          player_state[0] &= ~uint16_t(btn);
          IRGamepadUpdate(1, player_state[0]);
        }
